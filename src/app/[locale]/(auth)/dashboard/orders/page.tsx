...
const ORDER_STATUS_STYLES = {
  pending: {
    ...
    card: 'border-amber-300 bg-amber-50/80 shadow-sm',
    statusPanel: 'border-amber-400 bg-amber-100 text-amber-950',
  },
  confirmed: {
    section: 'border-sky-300 bg-sky-50 shadow-sm',
    badge: 'border-sky-400 bg-sky-100 text-sky-950',
    card: 'border-sky-300 bg-sky-50/80 shadow-sm',
    statusPanel: 'border-sky-400 bg-sky-100 text-sky-950',
  },
  preparing: {
    section: 'border-orange-300 bg-orange-50 shadow-sm',
    badge: 'border-orange-400 bg-orange-100 text-orange-950',
    card: 'border-orange-300 bg-orange-50/75 shadow-sm',
    statusPanel: 'border-orange-400 bg-orange-100 text-orange-950',
  },
  ready: {
...
const OrdersPage = async (props: {
  ...
  const orderQueryLimit = currentPage * ORDERS_PAGE_SIZE;
  ...
  const getOrdersHref = (period: string, page?: number) => {
    const params = new URLSearchParams({ period });

    if (period === 'custom') {
      params.set('from', customFromValue);
      params.set('to', customToValue);
    }
    ...
    return `${ordersPath}?${params.toString()}`;
  };
  const getExportHref = (period: string) => {
    const params = new URLSearchParams({ period });
  ...
  const canQueryOrders = selectedRange.isValid;
  ...
  const queriedOrders = canQueryOrders
    ? await db
      .select({
        id: orderSchema.id,
      ...
      .limit(orderQueryLimit + 1)
    : [];
  const hasMoreOrders = queriedOrders.length > orderQueryLimit;
  const orders = queriedOrders.slice(0, orderQueryLimit);
      ...
      <div className="rounded-md border bg-background/95 p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-2 lg:grid lg:grid-cols-1">
          {ORDER_PERIODS.filter(period => period !== 'custom').map(period => (
            ...
            >
              <Link href={getOrdersHref(period)}>
                {t(`period_${period}`)}
              </Link>
            </Button>
          ))}
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          {t('orders_limit_helper')}
        </p>
        {selectedPeriod === 'custom' && !selectedRange.isValid && (
      ...
      <div className="rounded-md border bg-muted/15 p-4">
        <div className="flex flex-wrap items-center gap-2 lg:grid lg:grid-cols-1">
          ...
          {ORDER_PERIODS.filter(period => period !== 'custom').map(period => (
            ...
            >
              <Link href={getExportHref(period)}>
                {t(`export_${period}`)}
              </Link>
            </Button>
          ))}
                    ...
                    {hasMoreOrders && (
                      <div className="mt-6 flex justify-center">
                        <Button asChild variant="outline">
                          <Link href={getOrdersHref(selectedPeriod, currentPage + 1)}>
                            {t('load_more_button')}
                          </Link>
                        </Button>
                      </div>
